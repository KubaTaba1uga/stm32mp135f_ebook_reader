...
Each command support passing a target, for example to clean uboot package you can do:

.. code-block:: console

   $ inv build-linux -t clean

Which under the hood executes ``make dirclean-linux``. More info about available buildroot targets and their usage can be find here:
...
We aim to replicate what Buildroot is doing manually in python (tasks.py), wich proves to be faster. Each command starts with ``fbuild-`` (abbreviation for “fast build”) and tries to do exactly what Buildroot does.
