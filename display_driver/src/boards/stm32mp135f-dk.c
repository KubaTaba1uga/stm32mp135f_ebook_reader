...
int dd_board_stm32mp135f_dk_init(dd_board_stm32mp135f_dk_t *out) {
  ...
  if (dd_gpio_pin_init(3, gpioh, &din) != 0) {
    dd_ewrap();
    goto error;
  }

  if (dd_gpio_pin_init(7, gpioh, &clk) != 0) {
    dd_ewrap();
    goto error;
  }

  if (dd_gpio_pin_init(11, gpioh, &cs) != 0) {
    dd_ewrap();
    goto error;
  }

  if (dd_gpio_pin_init(0, gpioi, &dc) != 0) {
    dd_ewrap();
    goto error;
  }

  if (dd_gpio_pin_init(2, gpioc, &rst) != 0) {
    dd_ewrap();
    goto error;
  }

  if (dd_gpio_pin_init(3, gpiog, &bsy) != 0) {
    dd_ewrap();
    goto error;
  }

  if (dd_gpio_pin_init(4, gpioa, &pwr) != 0) {
    dd_ewrap();
    goto error;
  }
